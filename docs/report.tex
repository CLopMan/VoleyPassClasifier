...
\pagenumbering{arabic} % numeración con múmeros arábigos para el resto de la publicación
    ...
    En este capítulo se describirán los difentes procesos llevados a cabo para la construcción y evaluación del modelo.

    \section{Filtrado de datos y etiquetado}

    Este proyecto se centrará en uno de los gestos técnicos, la colocación o
    armado, con el fin de reducir el alcance de la práctica. Los datos
    obtenidos de la primera base de datos han sido filtrados y reducidos a las
    secuencias de las clases \verb!r_set! o \verb!l_set!. Esto reduce el número
    de secuencias a 1276.
    ...
    hecho a través de un programa de \texttt{Python} (ver \ref{mod_ball}). Se ha escogido un 
    ...
    Para esto se ha usado un programa en \texttt{Python} (ver \ref{div_carpetas}).
    ...
    comprarán los resultados obtenidos empleando y sin emplear esta clase.
...
def modify_file(file_path, add_value=40):
   # Leer el contenido original del archivo
    with open(file_path, "r") as infile:
        lines = infile.readlines()

    # Sobrescribir el archivo con las lineas modificadas
    with open(file_path, "w") as outfile:
        for line in lines:
            line = line.strip()  # Elimina espacios o saltos de linea
            values = line.split()  # Divide la linea en valores separados por espacios
            ...
            if len(values) >= 2:  # Asegurarse de que haya al menos dos valores
                ...
                # Crear la nueva linea con los valores procesados y el valor annadido
...
def process_directories(base_directory, add_value=20):
     for root, dirs, files in os.walk(base_directory):
        for file in files:
            # Procesar solo los archivos de texto
...
# Valor que se annadira al final de cada linea
add_value = 20
...
def process_files_and_split_lines(base_directory):
    for root, dirs, files in os.walk(base_directory):
        ...
        file_index = 1  # Indice del archivo en la carpeta
        ...
        for file in files:
            ...
            if file.endswith(".txt"):  # Procesar solo archivos .txt
                ...
                # Escribir cada linea en un archivo separado
                for i, line in enumerate(lines):
                    ...
                    # Escribir la linea en el archivo correspondiente
                    with open(new_filepath, "w") as outfile:
                        outfile.write(line.strip() + "\n")
                    
                    print(f"Linea {i+1} del archivo '{file}' escrita en '{new_filename}'")
                
                file_index += 1  # Incrementar el indice del archivo en la carpeta
